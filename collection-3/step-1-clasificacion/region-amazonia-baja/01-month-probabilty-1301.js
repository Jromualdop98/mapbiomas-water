...
//             2023
//             ];
var regionId = [1301]  // NOTA: para exportar solo 1 region
var cloud_cover = 70; 
var version = 1  // exportar
var asset_name = 'clasificacion-02'// para exportar

var month = 9;   // visualizacion 
...
var modules = require('users/mapbiomasperu/mapbiomas-water:collection-2/modules/mosaic_amazonia-baja'); 
...
var gv_asc_Fit = ee.Dictionary(ee.List([[paramfuzzy.gv_asc_min,0],[paramfuzzy.gv_asc_max,1]]).reduce(ee.Reducer.linearFit()));
var gv_desc_Fit = ee.Dictionary(ee.List([[paramfuzzy.gv_desc_min,1],[paramfuzzy.gv_desc_max,0]]).reduce(ee.Reducer.linearFit()));
...
var snow_asc_Fit = ee.Dictionary(ee.List([[paramfuzzy.snow_asc_min,0],[paramfuzzy.snow_asc_max,1]]).reduce(ee.Reducer.linearFit()));
var snow_desc_Fit = ee.Dictionary(ee.List([[paramfuzzy.snow_desc_min,1],[paramfuzzy.snow_desc_max,0]]).reduce(ee.Reducer.linearFit()));
...
function class_1_probs (image) {
    ...
    var cond_4 = image.select('snow').multiply(snow_desc_Fit.getNumber('scale')).add(snow_desc_Fit.getNumber('offset')).clamp(0, 1)
                  .addBands(
                  image.select('snow').multiply(snow_asc_Fit.getNumber('scale')).add(snow_asc_Fit.getNumber('offset')).clamp(0, 1)  
                  ).reduce(ee.Reducer.min());
    
    // var cond_5 = image.select('gv').multiply(gv_Fit.getNumber('scale')).add(gv_Fit.getNumber('offset')).clamp(0, 1);
    var cond_5 = image.select('gv').multiply(gv_desc_Fit.getNumber('scale')).add(gv_desc_Fit.getNumber('offset')).clamp(0, 1)
                  .addBands(
                  image.select('gv').multiply(gv_asc_Fit.getNumber('scale')).add(gv_asc_Fit.getNumber('offset')).clamp(0, 1)  
                  ).reduce(ee.Reducer.min());
    var cond_6 = image.select('soil').multiply(soil_Fit.getNumber('scale')).add(soil_Fit.getNumber('offset')).clamp(0, 1);
    // var cond_6 = image.select('soil').multiply(soil_desc_Fit.getNumber('scale')).add(soil_desc_Fit.getNumber('offset')).clamp(0, 1)
    //               .addBands(
    //               image.select('soil').multiply(soil_asc_Fit.getNumber('scale')).add(soil_asc_Fit.getNumber('offset')).clamp(0, 1)  
    //               ).reduce(ee.Reducer.min());

    
    // var image_prob = cond_1.multiply(0.4).addBands(cond_5.multiply(0.3)).addBands(cond_6.multiply(0.3)) //.addBands(cond_3)
    //                       .reduce(ee.Reducer.sum()).rename('prob');

    var image_prob = cond_1.addBands(cond_2).addBands(cond_3) //.addBands(cond_3)
                          .reduce(ee.Reducer.mean()).rename('prob');

  
    image_prob = image_prob.where(image.select('soil').gt(10),0)
    // image_prob = image_prob.where(image.select('npv').gt(2),0)
    // image_prob = image_prob.where(image.select('gv').gt(35),0)
...
var water_y_m_func = function (year, moving_window,collection) {
  ...
  var detec = ee.Image(0).where(slope.gt(paramClasif.slope_corte).and(p_img_month.gte(paramClasif.p_img_month_gt)),1)
                        .where(slope.lte(paramClasif.slope_corte).and(p_img_month.gte(paramClasif.p_img_month_lte)),1)
...
for (var i=0; i<years.length; i++) {
  ...
  if(vis.Map_addLayer){
    Map.addLayer(ee.Geometry.Point(-73.23182, -3.67911)) 
...
var rios_quebradas = ee.FeatureCollection("projects/ee-mapbiomasperupr/assets/rios_quebradas")
Map.addLayer(rios_quebradas.style({color:'0000FF'}), {}, 'Rios - quebradas')

var lagunas = ee.FeatureCollection("projects/ee-jromualdoibc/assets/subestimacion_andes")
Map.addLayer(lagunas.style({color:'000000',pointSize: 3,}), {}, 'Lagunas Subestimación')
