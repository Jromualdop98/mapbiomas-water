...
//             2022
//             ];
var regionId = [1310]  // NOTA: para exportar solo 1 region
var cloud_cover = 70; 
...
function class_1_probs (image) {
    ...
    // var image_prob = cond_1.addBands(cond_5).addBands(cond_6) //.addBands(cond_3)
    //                       .reduce(ee.Reducer.mean()).rename('prob');

  
    image_prob = image_prob.where(image.select('soil').gt(25),0)
...
var water_y_m_func = function (year, moving_window,collection) {
  ...
  var remv = p_month.lt(0.2).selfMask();
