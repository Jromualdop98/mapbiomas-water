...
for (var i=0; i<years.length; i++) {
  ...
  var water_month_col = months.map(function (month) {
    ...
    var detet   = water_month.eq(1).multiply(1).selfMask().where(mask_freq.eq(1), 4)
                                                          .where(csf_year.lte(0.3).or(freq_year.lte(2).and(csf_year.lte(0.45))), 4)
                                                          // .where(ndwi_mcfeeters_year.lte(6000).and(MaskAndes.selfMask()), 4)  //MASK andes-costa ndwi_mcfeeters_year
                                                          .where(mndwi_year.lte(7800).and(MaskAndes.selfMask()), 4)  //MASK andes-costa ndwi_mcfeeters_year
                                                          .where(shade_year.lte(50), 4);
                                                          
    var fil_1   = water_month.eq(2).multiply(2).selfMask().where(mask_freq.eq(1), 4)
                                                          .where(csf_year.lte(0.3).or(freq_year.lte(2).and(csf_year.lte(0.45))), 4)
                                                          // .where(ndwi_mcfeeters_year.lte(6300).and(MaskAndes.selfMask()), 4)  //MASK andes-costa ndwi_mcfeeters_year
                                                          .where(mndwi_year.lte(8200).and(MaskAndes.selfMask()), 4)  //MASK andes-costa ndwi_mcfeeters_year
                                                          .where(shade_year.lte(50), 4);
